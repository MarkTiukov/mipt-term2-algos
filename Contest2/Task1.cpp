...
void createGraph(Graph &graph, int costA, int costB);
void dijkstra(Graph &graph, std::vector<int> &dist, int start);
...
void createGraph(Graph &graph, int costA, int costB) {
  for (long long i = 0; i < graph.size(); ++i) {
	long long next1 = (i + 1) % graph.size();
	long long next2 = (i * i + 1) % graph.size();
	if (next1 == next2) {
